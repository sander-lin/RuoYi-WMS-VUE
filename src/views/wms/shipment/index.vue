  ...
  <div class="app-container">
    ...
    <el-card class="mt20">
      ...
      <el-row :gutter="10" class="mb8" type="flex" justify="space-between">
        <el-col :span="6"><span style="font-size: large">发货管理</span></el-col>
        <el-col :span="1.5">
          <el-button
            type="primary"
            plain
      ...
      <el-table v-loading="loading" :data="shipmentList" border class="mt20">
        ...
        <el-table-column label="操作" align="right" class-name="small-padding fixed-width">
            <template #default="scope">
                <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['wms:shipment:edit']">修改</el-button>
                <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['wms:shipment:remove']">删除</el-button>
            </template>
        </el-table-column>
      </el-table>
    ...
    <el-drawer :title="title" v-model="open" size="50%" append-to-body>
      <el-form ref="shipmentRef" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="发货通知单编号" prop="shipmentNoticeId">
          <el-input v-model="form.shipmentNoticeId" placeholder="请输入发货通知单编号" />
        </el-form-item>
...
import { listShipment, getShipment, delShipment, addShipment, updateShipment } from "@/api/wms/shipment";
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除发货管理编号为"' + _ids + '"的数据项？').then(function() {
    loading.value = true;
...
function handleExport() {
  proxy.download('wms/shipment/export', {
    ...queryParams.value
