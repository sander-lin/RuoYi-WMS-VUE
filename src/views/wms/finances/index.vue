<template>
  <div class="app-container">
    <el-card>
      <el-form :model="queryParams" ref="queryRef" :inline="true" label-width="68px">
        ...
        <el-form-item label="账单子类型" prop="subType">
          <el-select v-model="queryParams.subType" placeholder="请选择账单子类型" clearable>
            <el-option
    ...
    <el-card class="mt20">
      ...
      <el-row :gutter="10" class="mb8" type="flex" justify="space-between">
        <el-row type="flex" justify="space-between">
          <el-col class="mb5" ><span style="font-size: large">资金明细</span></el-col>
          <el-col> <el-button
            type="primary"
            plain
            icon="Plus"
      ...
      <el-table v-loading="loading" :data="financesList" border class="mt20">
        ...
        <el-table-column label="账单子类型" prop="subType">
          <template #default="scope">
            <dict-tag :options="wms_finances_sub_type" :value="scope.row.subType"/>
          </template>
        </el-table-column>
        <el-table-column label="关联用户id" prop="userId" />
      </el-table>
    ...
    <el-dialog :title="title" v-model="open" size="50%" append-to-body>
      <el-form ref="financesRef" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="关联用户id" prop="userId">
...
import { listFinances, getFinances, delFinances, addFinances, updateFinances } from "@/api/wms/finances";

const { proxy } = getCurrentInstance();
const { wms_finances_sub_type, wms_finances_type } = proxy.useDict('wms_finances_sub_type', 'wms_finances_type');
  ...
  proxy.$refs["financesRef"].validate(valid => {
    if (valid) {
      ...
      if (form.value.id != null) {
        updateFinances(form.value).then(response => {
        ...
        addFinances(form.value).then(response => {
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除资金明细编号为"' + _ids + '"的数据项？').then(function() {
    loading.value = true;
...
function handleExport() {
  proxy.download('wms/finances/export', {
    ...queryParams.value
