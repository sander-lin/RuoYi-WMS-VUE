      ...
      >
        <el-form-item label="入库状态" prop="receiptOrderStatus">
          <el-radio-group
            ...
            @change="handleQuery"
          >
            <el-radio-button :key="-2" :label="-2"> 全部 </el-radio-button>
            <el-radio-button
        ...
        <!-- <el-form-item label="入库类型" prop="receiptOrderStatus">
          <el-radio-group
            ...
            @change="handleQuery"
          >
            <el-radio-button :key="-1" :label="-1"> 全部 </el-radio-button>
            <el-radio-button
      ...
      >
          ...
          <template #default="props">
            <div style="padding: 0 50px 20px 50px">
              ...
              <el-table
                ...
                v-loading="detailLoading[props.$index]"
                empty-text="暂无商品明细"
              >
                ...
                <el-table-column label="生产日期" prop="productionDate">
                  <template #default="{ row }">
                    <div>
                      {{ parseTime(row.productionDate, "{y}-{m}-{d}") }}
                    </div>
                  </template>
                </el-table-column>
        ...
        <el-table-column label="仓库/库区" align="left" width="200">
          <template #default="{ row }">
            ...
            <div v-if="row.areaId">
              库区：{{ useWmsStore().areaMap.get(row.areaId)?.areaName }}
            </div>
          </template>
        </el-table-column>
        <el-table-column
        ...
        >
          <template #default="{ row }">
            <dict-tag
              :options="wms_receipt_status"
              :value="row.receiptOrderStatus"
            />
          </template>
        </el-table-column>
        <el-table-column label="数量/金额(元)" align="left">
          <template #default="{ row }">
            ...
            <div
              ...
              v-if="row.payableAmount || row.payableAmount === 0"
            >
              <span>金额：</span>
        ...
        <el-table-column label="创建时间/操作时间" align="left" width="200">
          <template #default="{ row }">
            <div>
              创建：{{ parseTime(row.createTime, "{mm}-{dd} {hh}:{ii}") }}
            </div>
            <div>
              操作：{{ parseTime(row.updateTime, "{mm}-{dd} {hh}:{ii}") }}
            </div>
          </template>
        </el-table-column>
          ...
          <template #default="scope">
            <div>
              <el-popover
                ...
                :content="
                  '入库单【' +
                  scope.row.receiptOrderNo +
                  '】已' +
                  (scope.row.receiptOrderStatus === 1 ? '入库' : '作废') +
                  '，无法修改！'
                "
              >
                <template #reference>
                  <el-button
                    ...
                    v-hasPermi="['wms:receipt:all']"
                    :disabled="[-1, 1].includes(scope.row.receiptOrderStatus)"
                    >修改</el-button
                  >
                </template>
              </el-popover>
              <el-button
                ...
                v-hasPermi="['wms:receipt:all']"
                >{{
                  expandedRowKeys.includes(scope.row.id) ? "收起" : "查看"
                }}</el-button
              >
            </div>
            <div class="mt10">
              <el-popover
                ...
                :content="
                  '入库单【' + scope.row.receiptOrderNo + '】已入库，无法删除！'
                "
              >
                <template #reference>
                  <el-button
                    ...
                    v-hasPermi="['wms:receipt:all']"
                    :disabled="scope.row.receiptOrderStatus === 1"
                    >删除</el-button
                  >
                </template>
              </el-popover>
...
function handleDelete(row) {
  ...
  proxy.$modal
    .confirm("确认删除入库单【" + row.receiptOrderNo + "】吗？")
    .then(function () {
      loading.value = true;
...
async function handlePrint(row) {
  ...
  const printData = {
    ...
    receiptOrderType: proxy.selectDictLabel(
      wms_receipt_type.value,
      receiptOrder.receiptOrderType
    ),
    receiptOrderStatus: proxy.selectDictLabel(
      wms_receipt_status.value,
      receiptOrder.receiptOrderStatus
    ),
    merchantName: useWmsStore().merchantMap.get(receiptOrder.merchantId)
      ?.merchantName,
    orderNo: receiptOrder.orderNo,
    warehouseName: useWmsStore().warehouseMap.get(receiptOrder.warehouseId)
      ?.warehouseName,
    areaName: useWmsStore().areaMap.get(receiptOrder.areaId)?.areaName,
    totalQuantity: Number(receiptOrder.totalQuantity).toFixed(0),
    payableAmount:
      receiptOrder.payableAmount || receiptOrder.payableAmount === 0
        ? receiptOrder.payableAmount + "元"
        : "",
  ...
  let printTemplate = new proxy.$hiprint.PrintTemplate({
    ...
    .then((res) => {
      if (res.data?.length) {
        const details = res.data.map((it) => {
          return {
            ...it,
            warehouseName: useWmsStore().warehouseMap.get(it.warehouseId)
              ?.warehouseName,
