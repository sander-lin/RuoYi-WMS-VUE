<template>
  <div>
                ...
                >
                  <el-option
                    v-for="item in useWmsStore().areaList.filter(
                      (it) => it.warehouseId === form.warehouseId
                    )"
                    :key="item.id"
                    :label="item.areaName"
                    :value="item.id"
          ...
          <el-row :gutter="24">
            ...
            <el-col :span="6">
              <el-form-item label="客户" prop="merchantId">
                ...
                >
                  <el-option
                    v-for="item in useWmsStore().merchantList"
                    :key="item.id"
    ...
    <div class="footer-global">
      <div class="btn-box">
        <div>
          <el-button @click="doShipment" type="primary" class="ml10"
            >完成出库</el-button
          >
...
const close = () => {
  const obj = { path: "/inventory/shipmentOrder" };
  ...
  item.forEach((it) => {
    if (
      ...
      form.value.details.push({
        ...
        areaName: useWmsStore().areaMap.get(form.value.areaId ?? it.areaId)
          ?.areaName,
      });
    }
  });
